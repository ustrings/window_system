								...
								<c:if test="${not empty pager }">
									<c:forEach items="${pager.result }" var="userDto" varStatus="state">
										<tr class="tbody_tr"> 
						    				...
						    				<td>${userDto.companyLeader }</td> 
						    				<td>${userDto.telNbr }</td> 
						    				<td>${fn:substringBefore(userDto.stsDate,".0")}</td> 
						    				<td><input type="image" id="${userDto.userId }" name="editUser" src="/resources/images/icn_edit.png" title="编辑"><input type="image" src="/resources/images/icn_trash.png" title="删除" name="deleteUser" id="${userDto.userId }"><input type="image" src="/resources/images/icn_cert.png" title="查看证书"></td> 
										</tr>
									</c:forEach>
								</c:if> 
							</tbody> 
