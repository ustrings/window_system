 ...
 */
int http_parser_level_2(struct pfring_pkthdr * hdr, const u_char *buffer, struct http_request_kinfo *httprequest)
{;
    u_char ch, *t, *m = NULL;
    int i = 0;
 ...
 */
int http_parser_level_1(int thread_id, struct pfring_pkthdr * hdr, const u_char *buffer, struct http_request_kinfo *http)
{
    ...
    if(hdr->extended_hdr.parsed_pkt.tunnel.tunnel_id != NO_TUNNEL_ID)
    {
        ...
        int header_len = hdr->extended_hdr.parsed_pkt.offset.payload_offset+sizeof(struct compact_ip_hdr) + tcp->doff * 4;
        http->content = (u_char *) (buffer + header_len);
        http->len = hdr->caplen-header_len;


//        struct in_addr in;
//        in.s_addr = htonl(http->sip);
//        char * curr = inet_ntoa(in);
//
//        if(strcmp("101.86.114.120", curr) == 0)
//        {
//            printf("a 101.86.114.120 come ips=%ld, header =%d\n", http->sip, header_len);
//            http_parser_print_payload(buffer,hdr->len+10);
//        }
//        if(strcmp("114.84.143.45", curr) == 0)
//        {
//            printf("a 114.84.143.45 come ips=%ld, header =%d\n", http->sip, header_len);
//            http_parser_print_payload(buffer,hdr->len+10);
//        }
    }
    else
