...
{
	if(NULL == dir_process){return false;}
	CFileTable ** dir_table = dir_process->cfTable;
...
int dir_init( struct CDirProcess* dir_process )
{
	...
	if(dir_process->cfTable == NULL)
	{
		printf("create file table failed -- dir_init\n");
		return -1;
	}
