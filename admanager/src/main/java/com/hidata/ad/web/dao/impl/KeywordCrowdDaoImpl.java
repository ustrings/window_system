 ...
 */
@Component
public class KeywordCrowdDaoImpl implements KeywordCrowdDao {
	...
	public List<KeyWordCrowdDto> queryKeywordCrowdListByCondition(KeyWordCrowdDto keyWordCrowdDto) throws Exception{
		...
		sql.append("select  * ");
		sql.append(" from keyword_crowd ");
		sql.append(" where 1 = 1 ");
		
		List<Object> paramList = new ArrayList<Object>();
		
		if(!StringUtils.isEmpty(keyWordCrowdDto.getCrowdId())){
			sql.append(" and crowd_id=?");
			paramList.add(keyWordCrowdDto.getCrowdId());
		}
		
		return db.queryForListObject(sql.toString(), paramList.toArray(),KeyWordCrowdDto.class);
	}
}
