...
(function (factory) {
    ...
    Highcharts.Chart.prototype.getTable = function (useLocalDecimalPoint) {
        var html = "<table id ='reportId' class='table table-bordered table-hover table-condensed table-striped'>",
            rows = this.getDataRows();
        ...
        each(rows, function (row, i) {
            ...
            for (j = 0; j < row.length; j = j + 1) {
                ...
                if (typeof val === 'number') {
                    ...
                    val = Highcharts.numberFormat(val, 2,'.',',');
                    html += '<' + tag + ' class="number">' + val + '</' + tag + '>';

                } else {
                    html += '<' + tag + '>' + (val === undefined ? '0.00' : val) + '</' + tag + '>';
                }
            }
    ...
    function getContent(chart, extension, content, MIME) {
        var  url = mainDomain+'/report/exportReport';
    ...
    Highcharts.Chart.prototype.viewData = function () {
        if (!this.insertedTable) {
        	...
        	if(reportTableDiv){reportTableDiv.remove();}     	
