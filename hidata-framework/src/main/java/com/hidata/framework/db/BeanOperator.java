...
import com.hidata.framework.annotation.Param;
...
public class BeanOperator{
	...
	public static <T> String getSqlByObject(SqlTypes sqlType , T obj , String tableName){
		...
		String[] dateTypes = {"java.sql.Date","java.sql.Time","java.sql.Timestamp","java.util.Date"};
		...
		if(sqlType == SqlTypes.INSERT){
			...
			for(Field field : fields){
				...
				if (StringUtil.contains(intTypes, type.getName())){
						isNumber = true;
				} else if (StringUtil.contains(dateTypes, type.getName())){
					isDate = true;
				}
				...
				if (index == 1){
					...
					if(isNumber){
						...
						//TODO
						//values.append("str_to_date('"+value+"','%Y-%m-%d %H:%T:%s')");
						String dtStr = DateUtil.formatTime((java.util.Date) value);
						values.append("'").append(dtStr).append("'");
					} else {
...
//		System.out.println(BeanOperator.getSqlByObject(SqlTypes.INSERT, form, "orderform_unpay"));
